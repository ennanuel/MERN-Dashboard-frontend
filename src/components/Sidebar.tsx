      ...
      { 
        isSidebarOpen && (
          ...
          >
              ...
              <List>
                {
                  navItems.map( ({text, icon}, i) => (
                    !icon ?
                    <Typography key={i} sx={{m: "2.25rem 0 1rem 3rem"}}> {text} </Typography> :
                    <ListItem key={i}>
                      <ListItemButton 
                        onClick={() => { navigate(`/${text.toLowerCase()}`); setActive(text.toLowerCase()) }}
                        sx={{ 
            ...
            <Box sx={{positon: 'absolute'}} bottom="2rem">
              <Divider />
